...
public class JbehaveExtentReporter extends JbehaveReport {
    ...
    static {
        try {
            storyCancelled = StoryReporter.class.getMethod("storyCancelled",
                    Story.class, StoryDuration.class);
            storyNotAllowed = StoryReporter.class.getMethod("storyNotAllowed",
                    Story.class, String.class);
            beforeStory = StoryReporter.class.getMethod("beforeStory",
                    Story.class, Boolean.TYPE);
            afterStory = StoryReporter.class.getMethod("afterStory",
                    Boolean.TYPE);
            narrative = StoryReporter.class.getMethod("narrative",
                    Narrative.class);
            lifecycle = StoryReporter.class.getMethod("lifecyle",
                    Lifecycle.class);
            scenarioNotAllowed = StoryReporter.class.getMethod(
                    "scenarioNotAllowed", Scenario.class, String.class);
            beforeScenario = StoryReporter.class.getMethod("beforeScenario",
                    String.class);
            scenarioMeta = StoryReporter.class.getMethod("scenarioMeta",
                    Meta.class);
            afterScenario = StoryReporter.class.getMethod("afterScenario");
            givenStories = StoryReporter.class.getMethod("givenStories",
                    GivenStories.class);
            givenStoriesPaths = StoryReporter.class.getMethod("givenStories",
                    List.class);
            beforeExamples = StoryReporter.class.getMethod("beforeExamples",
                    List.class, ExamplesTable.class);
            example = StoryReporter.class.getMethod("example", Map.class);
            afterExamples = StoryReporter.class.getMethod("afterExamples");
            beforeStep = StoryReporter.class.getMethod("beforeStep",
                    String.class);
            successful = StoryReporter.class.getMethod("successful",
                    String.class);
            ignorable = StoryReporter.class.getMethod("ignorable",
                    String.class);
            pending = StoryReporter.class.getMethod("pending", String.class);
            notPerformed = StoryReporter.class.getMethod("notPerformed",
                    String.class);
            failed = StoryReporter.class.getMethod("failed", String.class,
                    Throwable.class);
            failedOutcomes = StoryReporter.class.getMethod("failedOutcomes",
                    String.class, OutcomesTable.class);
            dryRun = StoryReporter.class.getMethod("dryRun");
            pendingMethods = StoryReporter.class.getMethod("pendingMethods",
                    List.class);
            restarted = StoryReporter.class.getMethod("restarted", String.class,
                    Throwable.class);
            restartedStory = StoryReporter.class.getMethod("restartedStory",
                    Story.class, Throwable.class);
    ...
    public JbehaveExtentReporter(StoryReporter crossReferencing,
                                 StoryReporter delegate, boolean multiThreading) {
    ...
    public void storyNotAllowed(Story story, String filter) {
        ...
        if (multiThreading) {
            delayedMethods
            .add(new DelayedMethod(storyNotAllowed, story, filter));
        } else {
    ...
    public void beforeStory(Story story, boolean givenStory) {
        if (story.getName().equalsIgnoreCase("BeforeStories")
                || story.getName().equalsIgnoreCase("AfterStories")) {
            // TODO: Do something with beforeStories and AfterStories in the
            // future
        } else {
            ExtentReportManager.createTest(story.getName());
        }
        crossReferencing.beforeStory(story, givenStory);
        if (multiThreading) {
            delayedMethods
            .add(new DelayedMethod(beforeStory, story, givenStory));
        } else {
    ...
    public void scenarioNotAllowed(Scenario scenario, String filter) {
        ...
        if (multiThreading) {
            delayedMethods.add(
                    new DelayedMethod(scenarioNotAllowed, scenario, filter));
        } else {
    ...
    public void beforeScenario(String scenarioTitle) {
        ...
        if (sameScenarios.length > 1) {
            ...
            scenarioLabel.append("Scenario:").append("</br>")
            .append("&nbsp;&nbsp;").append("- ").append(scenarioTitle);
        }

        childLogger = ExtentReportManager.createScenarioOutlineOrStandard(scenarioLabel.toString(),"");
        crossReferencing.beforeScenario(scenarioTitle);
        if (multiThreading) {
            delayedMethods
            .add(new DelayedMethod(beforeScenario, scenarioTitle));
        } else {
    ...
    public void afterScenario() {
        ...
        if (scenarioFailed) {
           // this.childLogger.log(LogStatus.FAIL, this.failedReason);
    ...
    public void givenStories(List<String> storyPaths) {
        ...
        if (multiThreading) {
            delayedMethods
            .add(new DelayedMethod(givenStoriesPaths, storyPaths));
        } else {
    ...
    public void failed(String step, Throwable cause) {
        ...
        if (cause.getCause().getClass().toString().contains(SKIP_EXCEPTION)) {
            ...
            ExtentReportManager.addCucumberFailStep(cause);
        } else {
            this.scenarioFailed = true;
            failedReason = cause.getCause();
            ExtentReportManager.addCucumberFailStep(failedReason);
        }
    ...
    public void storyCancelled(Story story, StoryDuration storyDuration) {
        ...
        if (multiThreading) {
            delayedMethods.add(
                    new DelayedMethod(storyCancelled, story, storyDuration));
        } else {
    ...
    public void invokeDelayed() {
        ...
        synchronized (delegate) {
            for (DelayedMethod delayed : Collections
                    .unmodifiableList(delayedMethods)) {
