...
public abstract class BaseTest {
	...
	public void afterMethod(final ITestContext testContext, Method method, ITestResult result, @Optional String testDescription) {
		if (result.getStatus() == ITestResult.SUCCESS || result.getStatus() == ITestResult.SKIP || Integer.valueOf(PropertyManager.getProperty("repeat.test.failure")) <= (Integer) MyThreadLocal.get().getData(Context.RETRY_COUNT)) {
			if (testDescription != null && !testDescription.isEmpty()) {
		...
		switch (result.getStatus()) {
			case ITestResult.FAILURE:
				if (Integer.valueOf(PropertyManager.getProperty("repeat.test.failure")) <= (Integer) MyThreadLocal.get().getData(Context.RETRY_COUNT)) {
		...
		if ((result.getStatus() == ITestResult.SUCCESS) || (Integer.valueOf(PropertyManager.getProperty("repeat.test.failure")) <= (Integer) MyThreadLocal.get().getData(Context.RETRY_COUNT))) {
