package com.crowdar.zapi.jenkins.utils.rest;
		...
		for (Iterator<TestCaseResultModel> iterator = testCases.iterator(); iterator.hasNext();) {
			TestCaseResultModel testCaseWithStatus = (TestCaseResultModel) iterator
				...
				if (searchedTests.containsKey(testCaseWithStatus.getTestCaseName())) {
					...
					String testCase = testCaseWithStatus.getTestCase().toString();
					JSONObject issue = new JSONObject(testCase);
					fields = new JSONObject();
					fields.put("update", new JSONObject());
					fields.put("fields",issue);
					issueUpdates.put(fields);
...
//		}
	...
	public static void processTestCaseDetails(ZephyrConfigModel zephyrData) {
		...
		if (zephyrData.isZfjClud()) {
			if(zephyrData.getCycleIdZfjCloud() == null || zephyrData.getCycleName().equalsIgnoreCase(ZfjConstants.NEW_CYCLE_KEY)) {
			...
			if(zephyrData.getCycleId() == null || zephyrData.getCycleId() == NEW_CYCLE_KEY_IDENTIFIER.toString()) {
				cycleId = Cycle.createCycle(zephyrData).toString();
