...
echo "Checking for presence of OSSRH_USERNAME environment variable...."
if [[ $OSSRH_USERNAME ]]; then
  echo "Success - Found OSSRH Username in environment variables..."
else
  >&2 echo "Failure - OSSRH_USERNAME environment variable not set. Please set it and try again."
  exit 3
fi

echo "Checking for presence of OSSRH_PASSPHRASE environment variable...."
if [[ $OSSRH_PASSPHRASE ]]; then
  echo "Success - Found OSSRH Passphrase in environment variables..."
else
  >&2 echo "Failure - OSSRH_PASSPHRASE environment variable not set. Please set it and try again."
  exit 4
fi
...
if [ -f $TEMPLATE_FILE ]; then
   if cat $TEMPLATE_FILE | sed  "s/%NEXUS_USERNAME%/$NEXUS_USERNAME/g"  | sed "s/%NEXUS_PASSPHRASE%/$NEXUS_PASSPHRASE/g" | sed "s|\${env.NEXUS_USERNAME}|$NEXUS_USERNAME|g" | sed "s|\${env.NEXUS_PASSPHRASE}|$NEXUS_PASSPHRASE|g" | sed "s|\${env.OSSRH_USERNAME}|$OSSRH_USERNAME|g" | sed "s|\${env.OSSRH_PASSPHRASE}|$OSSRH_PASSPHRASE|g" | sed "s/generation text/ATTENTION: This file was generated by the create-settings.sh script.  Do not commit it to version control as it contains credentials!/" > $DESTINATION_FILE; then
